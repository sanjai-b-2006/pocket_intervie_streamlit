"""Independent 'hiring manager' powered by a Fireworks-hosted model on AMD Instinct GPUs.

This is deliberately a *different* provider and model from the Gemma 4 coach in ``llm.py``.
Gemma coaches the candidate; this Fireworks model plays the skeptical hiring manager who has to
actually make a call. Using two independent models means the final verdict is a genuine second
opinion, and it puts the hackathon's Fireworks/AMD credits to real work rather than leaving them
idle just because Gemma 4 itself isn't on Fireworks yet.
"""
...
class FireworksError(Exception):
    """Raised when the Fireworks judge call fails or returns an unusable response."""
...
def _extract_json(text: str) -> Dict[str, Any]:
    ...
    if not match:
        raise FireworksError(f"No JSON object found in Fireworks response: {text!r}")
    try:
        return json.loads(match.group(0))
    except json.JSONDecodeError as exc:
        raise FireworksError(f"Fireworks response was not valid JSON: {exc}") from exc
    ...
    def _chat(self, system: str, user: str, max_tokens: int = 1200) -> str:
        if not settings.judge_api_key:
            raise FireworksError(
                "No Fireworks judge key configured. Set JUDGE_API_KEY to enable the "
                "independent hiring verdict."
            )
        try:
            resp = httpx.post(
            ...
            raise FireworksError(
                f"Fireworks API returned {exc.response.status_code}: {exc.response.text[:300]}"
            ) from exc
        except httpx.RequestError as exc:
            raise FireworksError(f"Could not reach Fireworks API: {exc}") from exc
        ...
        # gpt-oss is a reasoning model: it can burn the whole token budget on hidden reasoning and
        # return an empty `content`. Surface that clearly instead of a downstream JSON parse error.
        if not content.strip():
            raise FireworksError(
                f"Fireworks returned empty content (finish_reason="
                f"{choice.get('finish_reason')!r}); likely truncated -- try again."
            )
        return content

    def hiring_verdict(self, session: InterviewSession) -> Dict[str, Any]:
        """Have the Fireworks model make an actual hire/no-hire call on the whole session."""
        transcript_block = "\n".join(
